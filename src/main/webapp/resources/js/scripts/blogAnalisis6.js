...
$(document).ready(function() {
	...
	if (typeof name  !== "undefined"){
	 ...
	 $("#favoritos").click(function(){
		 ...
		 getPostbyUserLike(usuario);
	 });	
	 //Comentario
	 $("#comentario").click(function(){
		 var usuario=$('#usuario',top.document).attr("value");
		 getUserPostComentAnalist(usuario);
	 });	
	 //edita alias
	 $('#editalias').click(function(event){
		   event.preventDefault();
		   document.location.href = "#ancla";
		$('#breadcrumb').html("<li><a href='#'>Blog</a></li><li><a href='active'>Editar Alias</a></li>");
	...
	var arraybodyPost = new Array();
		$.ajax({
			...
			$.each(data, function(i, obj) {
					...
					//var cadena=getListUserLike(obj.idPost);
					...
					$(function(){
						$("#form"+obj.idPost).ebcaptcha();
								...
								setDataPost(counterP,idPostP,imgAutorP,autorP,datefortP,tituloP,articuloP,showelementP,styleNotificationP,resultadosP,cadenaP,stylelikeme,megusta);
								...
								html5tooltips([
								               {
								               /*  contentText: cadena,*/
								                 targetSelector: "#likepost"+idPostP
								               },
								             ]);
							...
							name =$('#usuario',top.document).attr("name");
									  var jsonData = {
										...
										$("#saveUsername").click(function(){
											var taliasConfir=$('#confalias').val();
											show=1;
											var contratos=$('#usuario',top.document).attr('data-contratos');
...
function getComment(idPost){
	...
	$.ajax({
		...
		$.each(data2, function(i, obj) {
							...
							$('#comment'+obj.id_post).append("<div class='nomUserCom' id='nomusershow'>"+username+"</div><div class='usercoment'> "+obj.comment+"</div><div>"+datefort+"<div><hr>");
				});
				$('#comment2'+idPost).append("<form id='form"+idPost+"'><input type='button' id='enviar"+idPost+"' class='btn btn-primary commentx' value='Guardar'/ ></form>");
				$(function(){
				...
				$('#enviar'+idPost).click(function(){
							...
							//aqui se tiene q hacer una consulta para obtener el ALIAS
							...
							$.ajax({
								...
								alert("Estimado Usuario: Su comentario sera verificado para su publicación.");
								ALIASUSER=user;
							}).fail(function() {
	...
	$.ajax({
		...
		$.each(data6, function(i, obj) {
			//**consultas de incio
							...
							var datefort=formatDate(obj.datePublic);
							var megusta=false;
							setDataPost(counterP,idPostP,imgAutorP,autorP,datefort,tituloP,articuloP,showelementP,styleNotificationP,resultadosP,cadenaP,stylelikeme,megusta);
							$(function(){
								$("#form"+obj.idPost).ebcaptcha();
							});
...
function setAlias(user,name,nameTr,show){
	var contratos=$('#usuario',top.document).attr('data-contratos');
	var contrato; 
	if(contratos.indexOf(",")!=-1){
		contrato = contratos.substr(0,contratos.indexOf(","));
	}else{
		contrato=$('#usuario',top.document).attr('data-contratos');
	}
	var estatus=1;
...
function getPostLastFive(logeo){
	$.ajax({
...
function getOnlyOnePost(idPost,titulo,autor,imagen,articulo,fecha){
	...
	$("#likepost"+idPostP).click(function(){
		...
		/*	$(document).click(function(){
						$("#notificationContainer"+idPostP).hide();
						});
			//Popup Click
				$("#notificationContainer"+idPostP.click(function(){
					return false;
				});
...
function getPostbyWord(word){
	...
	$.ajax({
		...
		$.each(data12, function(i, obj) {
			...
			//$('#fivepostword').append("<li><a class='liga' name='MyLinks' id='liga"+obj.idPost+"'><p class='navbar-text'>"+obj.titulo+"</p></a></li> <br>");
		...
		//$( "a:contains('"+word+"')" ).css( "color", "red" );
		
	
		
		 //$("a:contains('Piojo')").addClass('test');
		
		
		/*
		var search = word;
		$(document).ready(function () {
		    $("div:fivepostword('"+search+"')").each(function () {
	...
	var username;
			  var jsonData = {
				...
				$.ajax({
					...
					/*}else{
						username =$('#usuario',top.document).attr("name");
...
function setDataPost(counterP,idPostP,imgAutorP,autorP,datefortP,tituloP,articuloP,showelementP,styleNotificationP,resultadosP,cadenaP,stylelikeme,megusta,cadena){
	var contenMegusta;
	if(megusta==true){
		contenMegusta="";
	}else{
		contenMegusta="<div class='txtMegusta'> " +
		"<label for='likeme'  id='megusta"+idPostP+"' "+showelementP+"  "+stylelikeme+">Me gusta</label>" +
		"</div>";
	}
	...
	/*$(".content2t").append("<div class='content"+counterP+"'><br>" +
			"<div  class='allcontent' id='p"+idPostP+"' "+cadenaP+">" +
					...
					"<div class='col-xs-12 allwhite'>"+
							"<div class='row'>"+
									"<span class='pull-left hidden-xs'>"+
											"<img src='"+imgAutorP+"' alt='analista' style='width: 95px;margin: 10px;border: 1px solid;'>"+
									"</span>"+
			        ...
			        "<div id='allComents"+idPostP+"'>" +
							...
							"<div id='collapse"+idPostP+"' class='panel-collapse collapse'>"+
									"<div class='panel-body'>"+
											"<div  id='comment"+idPostP+"' class='commentstl"+idPostP+"'>" +
											"</div>" +
									"</div>"+
									"<div id=addComent><div>" +
									"<input type='text' id='addComment"+idPostP+"' name='blah' class='inputcoment commentx' maxlength='1500' />" +
							"</div>" +
	...
	$(".content2t").append("<div class='content"+counterP+"'><br>" +
									"<div  class='allcontent' id='p"+idPostP+"' "+cadenaP+">" +
								        ...
								        "</div>"+	
											...
											"</div>"+
											"<div id=addComent>"+resultadosP+"<div><input type='text' id='addComment"+idPostP+"' name='blah' class='inputcoment' maxlength='1500' /></div>" +
											"<div id='txtMat'>Por favor responde esta sencilla respuesta matem\u00e1tica.</div>"+
											"<br>"+
											"<div style='margin-left: 5%'>"+
		...
		$.each(data12, function(i, obj) {
			...
			//obj.datePublic
			 ...
			 $('#liga'+obj.idPost).click(function(){
				...
				var megusta=true;
				 getOnlyOnePost(obj.idPost,obj.titulo,obj.autor,obj.imgAutor,obj.articulo,obj.datePublic,megusta);
				$('#fivepostword').hide();
			 });
			 $('#blog').click(function(){
					location.reload();
...
function getComentUser(idUser){
	...
	$.ajax({
		...
		/*	$('#fivepostword').append("<div  class='allconten'>"+
			
			    "<h3'>"+obj.titulo+"</h3><li><a class='panel-title' name='MyLinks' id='liga"+obj.idPost+"'>Ver Post</a></li>"+
			  "</div>"+
...
function getUserPostComentAnalist(idUser){
	$(".content2t").empty();
	$(".demo2").empty();
	$("#fivepostword").empty();
	var jsonData = {
	...
	$.ajax({
		url : 'BlogAnalisisController/getPostAnalisisAnalist',
		type : 'get',
		dataType : 'json',
		data : jsonData,
		async: true
	}).done(function(data25) {
			$('.breadcrumb').html("<li><a href='' id='blog'>Blog</a></li><li>Respuesta Analista</li>");
			if(data25.length!=0){
			 ...
			 $('#liga'+obj.idPost).click(function(){
				$(".liga").hide();
				setComentUserAnalist(idUser,obj.hora);
				 getOnlyOnePost(obj.idPost,obj.titulo,obj.autor,obj.imgAutor,obj.articulo,obj.datePublic);
				$('#fivepostword').hide();
			 });
			 $('#blog').click(function(){
...
function setComentUserAnalist(idUser,idUserComent){
	var jsonData = {
			'idUser': idUser,
			'idUserComent': idUserComent
		};
	$.ajax({
		url : 'BlogAnalisisController/setCommentAnalist',
		type : 'get',
		dataType : 'json',
		data : jsonData,
	}) .done(function(data){
			...
			$.each(data, function(i, obj) {
					...
					$(function(){
						$("#form"+obj.idPost).ebcaptcha();
								...
								cadenaP=cadena;
								var megusta=false;
								setDataPostSin(counterP,idPostP,imgAutorP,autorP,datefortP,tituloP,articuloP,cadenaP,megusta);
								$("#addComment"+obj.idPost).keyup(function() {
								    var $th = $(this);
								    $th.val($th.val().replace(/^\s*/, ''));
								    });
...
function setDataPostSin(counterP,idPostP,imgAutorP,autorP,datefortP,tituloP,articuloP,cadenaP,megusta){
	$(".content2t").append("<div class='content"+counterP+"'><br>" +
										"<div  class='allcontent' id='p"+idPostP+"' "+cadenaP+">" +
												"<div class='col-xs-12 allwhite'>"+
														"<div class='row'>"+
...
/*function setDataPostSin(counterP,idPostP,imgAutorP,autorP,datefortP,tituloP,articuloP,showelementP,styleNotificationP,resultadosP,cadenaP){
		...
		$.each(data2, function(i, obj) {
				...
				$('#enviar'+idPost).click(function(){
							...
							$.ajax({
								...
								if(user==null){
									alert("Estimado Usuario: Para ingresar su comentario favor de realizar Login");
									$('#pt_p1',top.document).modal('show');
								}else{
									alert("Estimado Usuario: Su comentario sera verificado para su publicación.");
...
/*function getListUserLike(idPost){
